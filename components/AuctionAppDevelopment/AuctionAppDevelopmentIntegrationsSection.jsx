...
import AuctionAppDevelopmentIntegrationIcon from "@/components/AuctionAppDevelopment/AuctionAppDevelopmentIntegrationIcons";
import {
  AUCTION_APP_DEVELOPMENT_INTEGRATIONS,
  AUCTION_INTEGRATION_ROW1,
  AUCTION_INTEGRATION_ROW2,
} from "@/lib/auctionAppDevelopmentIntegrations";
...
import bandStyles from "./AuctionAppDevelopmentIntegrationsSection.module.css";
        ...
        {slides.map((item) => (
          <SwiperSlide key={item._k} className="!w-auto">
            <div className="flex items-center gap-2.5 rounded-xl border border-neutral-200/90 bg-white px-4 py-2.5 shadow-sm sm:gap-3 sm:px-4 sm:py-3">
              <AuctionAppDevelopmentIntegrationIcon id={item.id} className="h-6 w-6 sm:h-7 sm:w-7" />
...
export default function AuctionAppDevelopmentIntegrationsSection() {
  const { subtitle, headline } = AUCTION_APP_DEVELOPMENT_INTEGRATIONS;

  return (
    <section
      className={`${bandStyles.section} auctionIntegrationsSectionMobileFlushTop`}
      aria-labelledby="auction-integrations-heading"
    >
      <div className={`${pageStyles.inner} auctionIntegrationsInnerFlush`}>
        <header className="mx-auto max-w-[900px] text-center">
          ...
          <h2
            id="auction-integrations-heading"
      ...
      <div className={pageStyles.integrationsMarqueeBleed}>
        <div className="mt-12 flex flex-col sm:mt-14 lg:mt-16" style={{ gap: MARQUEE_GAP_PX }}>
          <IntegrationMarqueeRow items={AUCTION_INTEGRATION_ROW1} reverse={false} />
          <IntegrationMarqueeRow items={AUCTION_INTEGRATION_ROW2} reverse />
        </div>
      </div>
    </section>
  );
