...
import ClassifiedPlatformIntegrationIcon from "@/components/ClassifiedPlatform/ClassifiedPlatformIntegrationIcons";
import {
  CLASSIFIED_PLATFORM_INTEGRATIONS,
  CLASSIFIED_PLATFORM_INTEGRATION_ROW1,
  CLASSIFIED_PLATFORM_INTEGRATION_ROW2,
} from "@/lib/classifiedPlatformIntegrations";
...
import bandStyles from "./ClassifiedPlatformIntegrationsSection.module.css";
        ...
        {slides.map((item) => (
          <SwiperSlide key={item._k} className="!w-auto">
            <div className="flex items-center gap-2.5 rounded-xl border border-neutral-200/90 bg-white px-4 py-2.5 shadow-sm sm:gap-3 sm:px-4 sm:py-3">
              <ClassifiedPlatformIntegrationIcon id={item.id} className="h-6 w-6 sm:h-7 sm:w-7" />
...
export default function ClassifiedPlatformIntegrationsSection() {
  const { subtitle, headline } = CLASSIFIED_PLATFORM_INTEGRATIONS;

  return (
    <section
      className={`${bandStyles.section} classifiedIntegrationsSectionMobileFlushTop`}
      aria-labelledby="classified-integrations-heading"
    >
      <div className={`${pageStyles.inner} classifiedIntegrationsInnerFlush`}>
        <header className="mx-auto max-w-[900px] text-center">
          ...
          <h2
            id="classified-integrations-heading"
      ...
      <div className={pageStyles.integrationsMarqueeBleed}>
        <div className="mt-12 flex flex-col sm:mt-14 lg:mt-16" style={{ gap: MARQUEE_GAP_PX }}>
          <IntegrationMarqueeRow items={CLASSIFIED_PLATFORM_INTEGRATION_ROW1} reverse={false} />
          <IntegrationMarqueeRow items={CLASSIFIED_PLATFORM_INTEGRATION_ROW2} reverse />
        </div>
      </div>
    </section>
  );
